...
import { newRental as action } from "@/server/db/utils";
...
function Inputs({ searchParams }: { searchParams: { [key: string]: string } }) {
  ...
  function updateParams(param: string, value: string) {
    ...
    router.replace(`/reserve/rent?${new URLSearchParams(newParams)}`, {
      scroll: false,
    });
  }
    ...
    <>
      ...
      <div className="flex flex-row gap-2">
        <select
          ...
          required
          value={formData.bike || ""}
          onBlur={(e) => updateParams("bike", e.target.value)}
          onChange={(e) => {
        ...
        <select
          ...
          onBlur={(e) => updateParams("days", e.target.value)}
          onChange={(e) => {
